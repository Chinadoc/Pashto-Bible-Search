 ...
 * Processes YouTube videos locally without Railway
 * 
 * Usage: node scripts/process-video-local.js <youtubeUrl> [elevenlabsApiKey]
 * Example: node scripts/process-video-local.js "https://www.youtube.com/watch?v=u9sU5l92Th4"
 */
...
async function transcribeWithElevenLabs(audioFile, apiKey) {
  ...
  try {
    ...
    if (error.response) {
      ...
      throw new Error(`ElevenLabs error: ${error.response.status} - ${errorText}`);
    }
    throw error;
  }
}
...
async function segmentTranscriptBySentences(text, videoDuration) {
  ...
  if (videoDuration > 0) {
    ...
    // Normalize to cover full duration
  ...
  for (let i = 0; i < segmentFiles.length; i++) {
    ...
    try {
      ...
      if (uploadResponse.ok) {
        ...
        console.warn(`Failed to upload segment ${i + 1}: ${errorText}`);
      }
    } catch (error) {
...
if (!youtubeUrl) {
  console.error('Usage: node scripts/process-video-local.js <youtubeUrl> [elevenlabsApiKey]');
  console.error('Example: node scripts/process-video-local.js "https://www.youtube.com/watch?v=u9sU5l92Th4"');
  process.exit(1);
}
