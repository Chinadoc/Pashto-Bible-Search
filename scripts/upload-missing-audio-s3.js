...
async function uploadFile(r2Key, filePath) {
    try {
        ...
        console.error(`\n❌ Failed to upload ${r2Key}:`, err.message);
        return false;
    }
}
...
async function uploadMissingAudio() {
    ...
    console.log('Identifying missing files...');
    ...
    for (const book of BOOKS) {
        for (let chapter = 1; chapter <= 28; chapter++) {
            ...
            for (const file of validFiles) {
                ...
                if (!r2Keys.has(r2Key)) {
                    uploadQueue.push({ r2Key, filePath: path.join(dirPath, file) });
                }
            }
        }
    }

    console.log(`\nFound ${uploadQueue.length} missing files. Starting upload with concurrency 20...`);
    ...
    for (let i = 0; i < uploadQueue.length; i += CONCURRENCY) {
        ...
        const promises = chunk.map(item => uploadFile(item.r2Key, item.filePath));
        const chunkResults = await Promise.all(promises);
        results.push(...chunkResults);

        // Update counts
        const success = chunkResults.filter(r => r).length;
        const fail = chunkResults.length - success;
        uploadedCount += success;
        errorCount += fail;

        process.stdout.write(` [${Math.min(i + CONCURRENCY, uploadQueue.length)}/${uploadQueue.length}] `);
    }
