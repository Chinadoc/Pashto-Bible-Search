...
async function rebuildWordIndex() {
  ...
  // Step 3: Convert to records with ONLY the columns that exist
  ...
  const records = Object.values(wordIndex).map(item => ({
    ...
    // Remove: base_word, is_inflected (don't exist in schema)
  }));
  ...
  for (const word of testWords) {
    ...
    if (data && data.length > 0) {
      ...
      for (const row of data) {
        ...
        console.log(`   ${row.translation_key}: ${row.frequency} verses, sample: ${verses.slice(0, 3).join(', ')}`);
      }
    } else {
