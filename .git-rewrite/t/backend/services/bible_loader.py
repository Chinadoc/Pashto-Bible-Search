...
def get_grammar_indices() -> Tuple[dict, dict]:
    ...
    # Prioritize the v15 grammatical index used by Streamlit app
    v15_index_path = os.path.join(APP_ROOT, 'grammatical_index_v15.json')
    if os.path.exists(v15_index_path):
        try:
            v15_data = _load_json(v15_index_path)
            # Assuming v15 has a similar structure we can adapt
            # This is a placeholder; we'll need to inspect v15_data's structure
            # For now, let's assume it can be used directly as the occurrence index
            form_occurrence_index = v15_data.get('form_occurrences', v15_data)
        except Exception:
            form_occurrence_index = {}
    else:
        foi_path = os.path.join(APP_ROOT, 'form_occurrence_index.json')
        try:
            form_occurrence_index = _load_json(foi_path) if os.path.exists(foi_path) else {}
        except Exception:
            form_occurrence_index = {}

    ftr_path = os.path.join(APP_ROOT, 'form_to_root_map.json')
    try:
        form_to_root_map = _load_json(ftr_path) if os.path.exists(ftr_path) else {}
    except Exception:
        form_to_root_map = {}
