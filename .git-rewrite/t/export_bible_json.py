...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument('--msgpack', action='store_true', help='Also write a MessagePack binary alongside JSON (pashto_bible.msgpack)')
    ...
    if args.msgpack:
        try:
            ...
            mp_path = os.path.splitext(out_path)[0] + '.msgpack'
            with open(mp_path, 'wb') as f:
                f.write(msgpack.packb(items, use_bin_type=True))
            size_mb = os.path.getsize(mp_path) / (1024 * 1024)
            print(f"Wrote {len(items):,} verses to {mp_path} ({size_mb:.2f} MB)")
        except Exception as e:
