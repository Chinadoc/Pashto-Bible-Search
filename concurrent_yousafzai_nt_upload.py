#!/usr/bin/env python3
"""
Concurrent Yousafzai New Testament audio uploader with multiple parallel processes.
"""
        ...
        for chapter_dir in book_dir.iterdir():
            ...
            for audio_file in chapter_dir.glob("yousafzai_*.mp3"):
                ...
                try:
                    ...
                    # Example: 1-corinthians001_verse_001
                    if len(parts) >= 3 and parts[-2] == 'verse':
                        # Find the verse part (last part should be a number)
                        verse_num = int(parts[-1])
                        
                        # The chapter is embedded in the first part (e.g., "1-corinthians001")
...
def main():
    ...
    print("🚀 Starting concurrent Yousafzai New Testament audio uploader...")
    ...
    if not files_to_upload:
        print("✅ No New Testament files to upload")
        return
    
    print(f"📁 Found {len(files_to_upload)} New Testament files to upload")
    ...
    print(f"📦 Processing {len(files_to_upload)} New Testament files with {num_workers} workers...")
