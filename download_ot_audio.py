#!/usr/bin/env python3
"""
Download OT audio files from Afghan Bibles for any specified book.

This script downloads chapter audio files and splits them into individual verses
for any Old Testament book.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ot_audio_download.log'),
...
class OTAudioDownloader:
    """Download and process OT audio files from Afghan Bibles"""

    BASE_URL = "https://afghanbibles.org"
    AUDIO_BASE_URL = f"{BASE_URL}/pashto-afeastern-audio"

    def __init__(self, audio_dir: str = "ot_audio_files", book_slug: str = "isaiah"):
        self.audio_dir = Path(audio_dir)
        self.book_slug = book_slug
        self.session = requests.Session()
        self.session.headers.update({
            'User-Agent': f'PashtoBibleSearch-{book_slug.title()}-Audio-Downloader/1.0 (+https://pashto-bible-search.vercel.app/)'
        })
    ...
    def get_chapter_count(self, book_slug: str) -> int:
        ...
        try:
            ...
            return chapter_counts.get(book_slug.lower(), 1)
        except Exception as e:
            logger.warning(f"Could not determine chapter count for {book_slug}: {e}")
            # Return book-specific defaults
            defaults = {
                'isaiah': 66,
                'psalms': 150,
                'genesis': 50,
            ...
            return defaults.get(book_slug.lower(), 1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download OT audio files from Afghan Bibles')
    parser.add_argument('book', help='Book name (e.g., isaiah, psalms, genesis)')
    ...
    downloader = OTAudioDownloader(args.audio_dir, args.book)

    asyncio.run(downloader.download_book_chapters(args.book, args.start_chapter, args.end_chapter))
