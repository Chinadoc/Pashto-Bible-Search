  ...
  try {
    ...
    const videos = (result.results || []).map((v: any) => {
      ...
      return {
        ...
        // Segments for ClientHome.tsx (handle both camelCase and snake_case)
        segments: segments.map((seg: any, i: number) => ({
          segmentNumber: seg.segmentNumber || seg.segment_number || i + 1,
          segment_number: seg.segmentNumber || seg.segment_number || i + 1,
          text: seg.text,
          transcript: seg.text,
          transcript_text: seg.text,
          startTime: seg.startTime ?? seg.start_time ?? 0,
          start_time: seg.startTime ?? seg.start_time ?? 0,
          endTime: seg.endTime ?? seg.end_time ?? 0,
          end_time: seg.endTime ?? seg.end_time ?? 0,
          duration: seg.duration || ((seg.endTime ?? seg.end_time ?? 0) - (seg.startTime ?? seg.start_time ?? 0)),
          audioUrl: seg.audioUrl || seg.audio_url || null,
          words: seg.words || [],
          type: 'segment',
        })),
        ...
        // Calculate total duration from last segment if not stored
        totalDuration: v.duration || (segments.length > 0 ? 
          (segments[segments.length - 1]?.endTime ?? segments[segments.length - 1]?.end_time ?? 0) : 0),
        total_duration: v.duration || (segments.length > 0 ? 
          (segments[segments.length - 1]?.endTime ?? segments[segments.length - 1]?.end_time ?? 0) : 0),
        duration: v.duration || (segments.length > 0 ? 
          (segments[segments.length - 1]?.endTime ?? segments[segments.length - 1]?.end_time ?? 0) : 0),
