-- Final comprehensive cleanup: Remove ALL punctuation and merge
-- This script processes entries in smaller batches to avoid timeouts

-- Step 1: Update all entries to remove punctuation in place
...
-- Step 2: Delete empty entries
DELETE FROM word_frequencies WHERE pashto_word = '' OR TRIM(pashto_word) = '';

-- Step 3: Delete punctuation-only entries
DELETE FROM word_frequencies WHERE pashto_word IN ('.', '،', ',', '!', '?', '؟') OR LENGTH(TRIM(pashto_word)) = 0;

-- Step 4: Merge duplicates - update first occurrence with sum of frequencies
