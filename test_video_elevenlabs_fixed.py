#!/usr/bin/env python3
"""
Process video with ElevenLabs (better Pashto support)
Downloads video → Converts to MP3 → Transcribes with ElevenLabs → Stores all segments in D1 → Uploads to R2
"""
...
def transcribe_with_elevenlabs(audio_file: str) -> str:
    """Transcribe audio file using ElevenLabs (better Pashto support)"""
    print('\n🎤 Step 2: Transcribing with ElevenLabs (Pashto)...')
...
def segment_transcript_by_sentences(text: str, audio_duration: float = None) -> List[Dict]:
    """Segment transcript into clips by sentences with time estimates"""
    print('\n✂️ Step 3: Segmenting transcript by sentences...')
    
    import re
    
    # Split by Pashto/Arabic sentence endings
    ...
    # Estimate duration per word (rough estimate: 0.4 seconds per word for Pashto)
    for sentence in sentences:
        if not sentence:
            continue
            
        word_count = len(sentence.split())
        estimated_duration = max(3, min(15, word_count * 0.4))  # 3-15 seconds
...
def store_metadata_in_d1(segments: List[Dict], video_id: str, transcript: str):
    """Store metadata in Cloudflare D1 via Worker - store ALL segments"""
    print('\n💾 Step 6: Storing metadata in Cloudflare D1...')
    
    try:
        print(f'   Storing {len(segments)} segments...')
        ...
        response = requests.post(
            ...
            json={
                ...
                'segments': segments,  # Send ALL segments
        ...
        if response.ok:
            result = response.json()
            print('✅ Metadata stored in D1 successfully!')
            print(f'   Video ID: {result.get("videoId", video_id)}')
            stored_segments = result.get("segments", [])
            print(f'   Segments stored: {len(stored_segments)}')
            print(f'   R2 Keys: {len(result.get("r2Keys", []))}')
            
            # Verify segments were stored correctly
            if len(stored_segments) != len(segments):
                print(f'   ⚠️ WARNING: Expected {len(segments)} segments, got {len(stored_segments)}')
        else:
...
def main():
    print('🎬 Starting ElevenLabs video processing (Better Pashto Support)...\n')
    ...
    try:
        ...
        # Step 2: Transcribe with ElevenLabs (better Pashto support)
        transcript = transcribe_with_elevenlabs(audio_file)
        
        # Step 3: Segment by sentences
        segments = segment_transcript_by_sentences(transcript)
        ...
        # Step 6: Store metadata in D1 (with ALL segments)
        store_metadata_in_d1(segments, VIDEO_ID, transcript)
