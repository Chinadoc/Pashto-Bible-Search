...
# No explicit firebase_admin.initialize_app() is needed here.
# db = firestore.client() # MOVED: This will be initialized within each function
...
def search_grammar(req: https_fn.Request) -> https_fn.Response:
    ...
    for doc in verses_ref.stream():
        verse = doc.to_dict()
        verse['ref'] = doc.id # Add the document ID as the ref
