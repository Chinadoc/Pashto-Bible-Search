...
import { presentEndings, pastEndings, imperativeEndings, baParticle, equativeEndings, emptyVerbBlock, passiveStativeBridge, } from "./grammar-units";
import { addToForm, mapVerbBlock, concatPsString, allOnePersonVerbForm, complementInflects, concatInflections, unisexInfToObjectMatrix, allOnePersonInflection, psStringEquals, } from "./p-text-helpers";
...
import { checkForIrregularConjugation, stativeAux, } from "./irregular-conjugations";
import { chooseParticipleInflection, spaceInForm, noPersInfs, ensureNonComboVerbInfo, ensureVerbConjugation, } from "./misc-helpers";
const dummyEntry = {
    i: 0,
    p: "",
    f: "",
...
export function conjugateVerb(entry, complement, verbInfo) {
    ...
    if (info.type === "transitive or grammatically transitive simple") {
        return {
            info,
            transitive: ensureVerbConjugation(conjugateVerb({ ...entry, c: entry.c ? entry.c.replace("/gramm. trans.", "") : "" }, dummyEntry, info.transitive)),
            grammaticallyTransitive: ensureVerbConjugation(conjugateVerb({ ...entry, c: entry.c ? entry.c?.replace("trans./", "") : "" }, dummyEntry, info.grammaticallyTransitive)),
        };
    }
    if (info.type === "dynamic or stative compound" ||
        info.type === "dynamic or generative stative compound") {
        return {
            info,
            stative: ensureVerbConjugation(conjugateVerb({ ...entry, c: entry.c ? entry.c.replace("dyn./", "") : "" }, dummyEntry, info.stative)),
            dynamic: ensureVerbConjugation(conjugateVerb({ ...entry, c: entry.c ? entry.c.replace("/stat.", "") : "" }, dummyEntry, info.dynamic)),
        };
    }
    ...
    const conjugation = {
        ...
        ...("singularForm" in info
            ? {
                singularForm: ensureVerbConjugation(conjugateVerb(entry, complement, info.singularForm)),
            }
            : {}),
...
function conjugateDynamicCompound(info) {
    // const willUseImperative = !(
    //     info.type === "dynamic compound"
    //     && info.transitivity === "intransitive"
    //     && info.auxVerb.p === "کېدل"
    // );
    const auxConj = enforceObject(ensureVerbConjugation(conjugateVerb(info.auxVerb, info.auxVerbComplement)), info.objComplement.person);
    ...
    const makeAspectContent = (aspect) => {
        const makeDynamicModalContent = () => {
    ...
    const participle = {
        ...
        past: ("long" in auxPPart && "masc" in auxPPart.long) || "masc" in auxPPart
            ? // @ts-ignore
                concatInflections(complement, auxPPart)
            : // @ts-ignore
                concatPsString(complement, " ", auxPPart),
    };
    const makePerfect = (pset) => ({
    ...
    const makePassiveAspectContent = (aspect, passive) => {
...
function makeAspectContent(info, aspect) {
    ...
    const roughPast = addToForm([root], pastEndings);
...
function makeJoinedModalContent(info, aspectIn) {
    const aspect = noPerfectiveModal(info) ? "imperfective" : aspectIn;
    ...
    const rootAndTail = aspect === "imperfective"
        ? rAndT.map((x) => accentImperfectiveModalRootAndTail(info, x))
        : rAndT;
    const nonImperative = addToForm([rootAndTail, " "], aux.nonImperative);
    const future = addToForm([baParticle, " "], nonImperative);
    const past = addToForm([rootAndTail, " "], 
    // @ts-ignore
    aux.past.short);
    const habitualPast = addToForm([baParticle, " "], past);
    function mhp(rt) {
...
function makeStativeCompoundSeperatedAspectContent(info, aspect) {
    const transitivity = getTransitivity(info);
    const complement = aspect === "imperfective"
    ...
    const nonImperative = addToForm([presentComplement, " "], stativeAux[transitivity][aspect].nonImperative);
    const future = addToForm([baParticle, " "], nonImperative);
...
function makeHypotheticalContent(info) {
    function makeStativeCompoundSepHypotheticalContent(info) {
        const transitivity = getTransitivity(info);
    ...
    const makeHypothetical = (root, length) => {
        if ("mascSing" in root) {
            // BIG TODO: SHOULD THERE BE PERS INFS HERE?? IGNORING THEM NOW IF THEY EXIST
            return makeHypothetical(root.mascSing, length);
        }
        return [
            accentOnNFromEnd(concatPsString(root[length], aayTail[0]), (length === "long" ? 1 : 0) + (info.yulEnding ? 1 : 0)),
            accentOnNFromEnd(concatPsString(root[length], aayTail[1]), (length === "long" ? 1 : 0) + (info.yulEnding ? 1 : 0)),
        ];
    };
    const short = makeHypothetical(info.root.imperfective, "short");
    const long = makeHypothetical(info.root.imperfective, "long");
    return {
        short: [
            [short, short],
            [short, short],
...
function makeParticipleContent(info) {
    const transitivity = getTransitivity(info);
    const past = "complement" in info
        ? concatInflections(info.complement, stativeAux[transitivity].participle.past)
        : "objComplement" in info
            ? concatInflections(info.objComplement.plural
                ? info.objComplement.plural
                : info.objComplement.entry, stativeAux[transitivity].participle.past)
            : inflectYay(noPersInfs(info.participle.past));
    const present = "complement" in info && spaceInForm(info.root.imperfective)
        ? concatInflections(info.complement, stativeAux[transitivity].participle.present)
...
function makePerfectContent(info) {
    const transitivity = getTransitivity(info);
    const pastPart = info.type === "stative compound"
...
function makePassiveContent(info) {
    function makePassiveAspectContent(aspect) {
        if ("complement" in info && spaceInForm(info.root[aspect])) {
            ...
            const nonImperative = addToForm([info.complement, " ", bridge, " "], stativeAux.intransitive[aspect].nonImperative);
            const future = addToForm([baParticle, " "], nonImperative);
            const past = addToForm([info.complement, " ", bridge, " "], stativeAux.intransitive[aspect].past);
            const habitualPast = addToForm([baParticle, " "], past);
            const modal = makePassiveModalSection([noPersInfs(info.root.imperfective).long, " "], stativeAux.intransitive.imperfective.modal);
            return {
                imperative: undefined,
                nonImperative,
                future,
    ...
    const simpleVerbParticiple = {
        past: concatPsString(removeAccents(noPersInfs(info.root.imperfective).long), " ", stativeAux.intransitive.info.participle.past),
        present: { p: "ن ا", f: "n / a" },
    };
...
function makePassiveModalSection(base, auxModal) {
    return {
...
function makePassivePerfectContent(info) {
    const pPart = stativeAux.intransitive.participle.past;
    // will always be transitive
    const halfPerfect = addToForm([info.complement, " ", passiveStativeBridge, " ", pPart], emptyVerbBlock);
    const past = addToForm([info.complement, " ", passiveStativeBridge, " ", pPart, " "], equativeEndings.past.short);
    const present = addToForm([info.complement, " ", passiveStativeBridge, " ", pPart, " "], equativeEndings.present);
    const habitual = addToForm([info.complement, " ", passiveStativeBridge, " ", pPart, " "], equativeEndings.habitual);
    const subjunctive = addToForm([info.complement, " ", passiveStativeBridge, " ", pPart, " "], equativeEndings.subjunctive);
    ...
    const pastSubjunctive = addToForm([info.complement, " ", passiveStativeBridge, " ", pPart, " "], equativeEndings.pastSubjunctive);
...
function enforceObject(conj, person) {
    const modifyPastInAspect = (as) => ({
...
// 2ND LEVER HELPERS
function finishSimpleVerbPast(info, aspect, roughPast) {
    const applyAccent = (block, form) => mapVerbBlock((item, rowNum, colNum) => {
        const nonRedundantLEnding = rowNum === 4 &&
            colNum === 1 &&
            item.p.slice(-1) === "ل" &&
            ["ul", "úl"].includes(item.f.slice(-2));
        const n = (form === "short" || nonRedundantLEnding ? 0 : 1) +
            (info.yulEnding ? 1 : 0);
...
function ensureShort3rdPersMascSing(info, aspect, block) {
    const replace3rdPersMascSing = (replacement, block) => [...block.slice(0, 4), [replacement, block[4][1]], block[5]];
    const makeAawuForm = (root) => {
        const base = {
            p: root.p.slice(0, -1),
            f: root.f.slice(0, -2),
        };
    ...
    const endsInAwul = ["awul", "awúl"].includes(infinitive.f.slice(-4)) &&
    ...
    if (info.idiosyncraticThirdMascSing) {
        ...
        const replacement = endsInAConsonant
            ? [
                form,
...
function accentImperfectiveModalRootAndTail(info, rt, length) {
    if ("long" in rt) {
        return {
            short: accentImperfectiveModalRootAndTail(info, rt.short, "short"),
            long: accentImperfectiveModalRootAndTail(info, rt.long, "long"),
        };
    }
    const n = info.yulEnding ? 2 : length === "short" ? 0 : 1;
    return accentOnNFromEnd(rt, n);
}
function getTransitivity(info) {
    return "transitivity" in info && info.transitivity === "intransitive"
        ? "intransitive"
        : "transitive";
}
function noPerfectiveModal(info) {
