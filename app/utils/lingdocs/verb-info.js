 ...
 *
 */
import { concatPsString, psStringEquals, removeEndingL, yulEndingInfinitive, removeRetroflexR, unisexInfToObjectMatrix, complementInflects, beginsWithDirectionalPronoun, checkForOoPrefix, removeStartingTick, ensureShortWurShwaShift, choosePersInf, isUnisexSet, getLong, getShort, } from "./p-text-helpers";
import { makePsString, removeFVarients } from "./accent-and-ps-utils";
import { inflectYay } from "./pashto-inflector";
import { accentOnFront, accentOnNFromEnd, removeAccents, } from "./accent-helpers";
import { inflectWord } from "./pashto-inflector";
import { checkForIrregularConjugation, kedulStat, stativeAux, } from "./irregular-conjugations";
import { presentParticipleSuffix } from "./grammar-units";
import { dynamicAuxVerbs } from "./dyn-comp-aux-verbs";
import { getPersonInflectionsKey, getPersonNumber, spaceInForm, getAuxTransitivity, chooseParticipleInflection, noPersInfs, ensureNonComboVerbInfo, } from "./misc-helpers";
import { isTlulVerb } from "./type-predicates";
const ayEndingUnaccented = { p: "ی", f: "ay" };
/**
...
export function getVerbInfo(ent, complmnt) {
    ...
    if (type !== "simple") {
        ...
        if (type === "dynamic or stative compound") {
            return {
                ...
                // @ts-expect-error (will have entry.c)
                { ...entry, c: entry.c.replace("dyn./stat.", "dyn.") }, complement),
                stative: getVerbInfo(
                // @ts-expect-error (will have entry.c)
                { ...entry, c: entry.c.replace("dyn./stat.", "stat.") }, complement),
            };
        }
        if (type === "dynamic or generative stative compound") {
            return {
                ...
                dynamic: getDynamicCompoundInfo(
                // @ts-expect-error (will have entry.c)
                { ...entry, c: entry.c.replace("gen. stat./dyn.", "dyn.") }, complement),
                stative: getGenerativeStativeCompoundVerbInfo(
                // @ts-expect-error (will have entry.c)
                { ...entry, c: entry.c.replace("gen. stat./dyn.", "gen. stat.") }, complement),
            };
        }
    ...
    const infinitive = "mascSing" in root.imperfective
...
function getGenerativeStativeCompoundVerbInfo(entry, comp, forceSingular) {
    const transitivity = getTransitivity(entry);
    ...
    const bases = {
        ...
        participle: {
            ...
            past: chooseParticipleInflection(inflectYay("mascSing" in auxVerb.info.participle.past
                ? // purely for type saftey, will not have mascSing
                    // in a non stative compound verb
                    /* istanbul ignore next */
                    auxVerb.info.participle.past.mascSing
                : auxVerb.info.participle.past), objComplement.person),
        },
    };
    ...
    const perfectiveRoot = concatPsString(compUsed, " ", bases.root.perfective);
    const root = {
        imperfective: concatPsString(compUsed, " ", bases.root.imperfective),
...
function getDynamicCompoundInfo(entry, comp, forceSingular) {
    const transitivity = getTransitivity(entry);
    const yulEnding = null;
    const objComplement = getObjComplementInfo(entry, comp, forceSingular);
    const auxVerb = getDynamicAuxVerb(entry);
    const auxVerbInfo = ensureNonComboVerbInfo(getVerbInfo(auxVerb.entry, auxVerb.complement));
    const compUsed = objComplement.plural
        ? objComplement.plural
        : objComplement.entry;
    const bases = auxVerbInfo.type === "stative compound"
        ...
        : {
            ...
            participle: {
                ...
                past: chooseParticipleInflection(inflectYay("mascSing" in auxVerbInfo.participle.past
                    ? // purely for type saftey, will not have mascSing
                        // in a non stative compound verb
                        /* istanbul ignore next */
                        auxVerbInfo.participle.past.mascSing
                    : auxVerbInfo.participle.past), objComplement.person),
            },
        };
    const stem = {
        imperfective: concatPsString(compUsed, " ", bases.stem.imperfective),
    ...
    const root = {
        imperfective: concatPsString(compUsed, " ", bases.root.imperfective),
        perfective: concatPsString(compUsed, " ", bases.root.perfective),
    ...
    const makeIntransitiveFormOfEntry = (e) => ({
        ...e,
...
function getObjectMatchingBases(auxInfo, person) {
    const key = getPersonInflectionsKey(person);
    const getBase = (x) => ("mascSing" in x ? x[key] : x);
    return {
        stem: {
...
function getObjComplementInfo(entry, complement, forceSingular) {
    const complementInEntry = makePsString(entry.p.split(" ")[0], entry.f.split(" ")[0]);
    const usesSeperatePluralForm = !forceSingular &&
        !psStringEquals(makePsString(complementInEntry.p, removeAccents(complementInEntry.f)), makePsString(complement.p, removeAccents(complement.f)));
    return {
        entry: complement,
...
export function getTransitivity(entry) {
    if (!entry.c) {
...
function getIdiosyncraticThirdMascSing(entry) {
...
function getVerbRoots(entry, transitivity, complement) {
    ...
    const imperfective = (() => {
        ...
        if (complement && spaceInForm(entry)) {
            ...
            return concatPsString(removeAccentsFull(comp), " ", aux);
        }
    ...
    const { perfective, pSplit, fSplit } = (() => {
        ...
        if (complement) {
            ...
            return {
                pSplit: 0,
                fSplit: 0,
                perfective: concatPsString(comp, " ", aux),
            };
        }
        ...
        return {
            perfective: shortAndLong(ps, "perfective"),
            pSplit,
            fSplit,
        };
    })();
 ...
 * @param entry - the dictionary entry for the verb
 */
function getVerbStems(entry, root, transitivity, complement) {
    function isRegEdulTransitive() {
        /* istanbul ignore next */
    ...
    const imperfective = (() => {
        ...
        if (complement && spaceInForm(root.imperfective)) {
            const comp = complementInflects(complement)
                ? unisexInfToObjectMatrix(complement)
                : complement.masc[0][0];
            return concatPsString(removeAccentsFull(comp), " ", stativeAux[auxTransitivity].info.stem.imperfective);
        }
    ...
    const { perfective, pSplit, fSplit } = (() => {
        ...
        const res = addOoPrefix(imperfective, entry);
        return {
            perfective: res.ps,
            pSplit: res.pSplit,
            fSplit: res.fSplit,
...
function splitPerfective(perfective, pSplit, fSplit, isStativeComp) {
    ...
    if ("long" in perfective) {
        return {
            short: splitPerfective(perfective.short, pSplit, fSplit, isStativeComp),
            long: splitPerfective(perfective.long, pSplit, fSplit, isStativeComp),
            ...("mini" in perfective
                ? {
    ...
    if (isStativeComp) {
        ...
        const before = makePsString(pWords.slice(0, -1).join(" ") + " ", fWords.slice(0, -1).join(" ") + " ");
        const after = makePsString(pWords[pWords.length - 1], fWords[fWords.length - 1]);
        return [before, after];
    }
    const pBeg = perfective.p.slice(0, pSplit);
    const before = makePsString(pBeg.endsWith(" ") ? pBeg.slice(0, -1) : pBeg, perfective.f.slice(0, fSplit).replace("`", ""));
    ...
    return [beforeAccented, after];
}
function getParticiple(entry, stem, infinitive, transitivity, complement) {
    const shortParticipleRoot = (() => {
        const shortenableEndings = ["ښتل", "ستل", "وتل"];
        // special thing for اېښودل - پرېښودل
        if (infinitive.p.slice(-4) === "ښودل" &&
    ...
    const makeSepStativePart = (complement, tense) => {
        const compInflects = complementInflects(complement);
        const comp = compInflects
            ? unisexInfToObjectMatrix(complement)
            : complement.masc[0][0];
        const aux = stativeAux[auxTransitivity].info.participle[tense];
        return concatPsString(removeAccentsFull(comp), " ", compInflects
            ? unisexInfToObjectMatrix(inflectYay(aux))
            : aux);
    };
    const accentPastPart = (pp) => accentOnNFromEnd(pp, yulEndingInfinitive(infinitive) ? 2 : 1);
    const auxTransitivity = getAuxTransitivity(transitivity);
    const past = entry.pprtp && entry.pprtf
        ...
        : complement
            ...
            : shortParticipleRoot
                ? {
                    short: accentPastPart(concatPsString(ensureShortWurShwaShift(shortParticipleRoot), ayEndingUnaccented)),
    ...
    const accentPresPart = (pp) => accentOnNFromEnd(pp, 1);
    const present = complement && spaceInForm(infinitive)
        ...
        : shortParticipleRoot &&
            (!psStringEquals(shortParticipleRoot, shortImperfectiveRoot) ||
                entry.p === "وتل")
            ? {
                short: accentPresPart(concatPsString(shortParticipleRoot, presentParticipleSuffix)),
                long: accentPresPart(concatPsString(shortImperfectiveRoot, presentParticipleSuffix)),
            }
            : "short" in stem.imperfective &&
                entry.shortIntrans &&
                entry.p !== "اوسېدل"
                ? {
                    short: accentPresPart(concatPsString(stem.imperfective.short, presentParticipleSuffix)),
                    long: accentPresPart(concatPsString(shortImperfectiveRoot, presentParticipleSuffix)),
                }
                : accentPresPart(concatPsString(shortImperfectiveRoot.p === "وړ"
                    ? ensureShortWurShwaShift(shortImperfectiveRoot)
                    : shortImperfectiveRoot, presentParticipleSuffix));
    return {
        past,
        present,
    };
 ...
 * @param entry - the dictionary entry for the verb
 */
function addOoPrefix(s, entry) {
    let pSplit = 0;
    let fSplit = 0;
    function attachOo(ps) {
        if ("long" in ps) {
            return {
...
function getDynamicAuxVerb(entry) {
    const auxWord = entry.p.trim().split(" ").slice(-1)[0];
    const auxWordResult = dynamicAuxVerbs.find((a) => a.entry.p === auxWord);
    /* istanbul ignore next */
    if (!auxWordResult) {
        throw new Error(`${entry.p} - ${entry.ts} unknown auxilary verb ${auxWord} for dynamic compound`);
    }
    return {
...
function getComplementPerson(complement, usesSeperatePluralForm) {
    const number = (complement.c && complement.c.includes("pl.")) || usesSeperatePluralForm
        ? "plural"
        : "singular";
    const gender = complement.c && complement.c.includes("n. m.") ? "masc" : "fem";
    return getPersonNumber(gender, number);
}
function makeDynamicPerfectiveSplit(comp, auxSplit) {
    if ("mascSing" in auxSplit) {
        return {
            mascSing: makeDynamicPerfectiveSplit(comp, auxSplit.mascSing),
            mascPlur: makeDynamicPerfectiveSplit(comp, auxSplit.mascPlur),
            femSing: makeDynamicPerfectiveSplit(comp, auxSplit.femSing),
            femPlur: makeDynamicPerfectiveSplit(comp, auxSplit.femPlur),
        };
    }
    if ("long" in auxSplit) {
        return {
            long: makeDynamicPerfectiveSplit(comp, auxSplit.long),
            short: makeDynamicPerfectiveSplit(comp, auxSplit.short),
            ...(auxSplit.mini
                ? {
                    mini: makeDynamicPerfectiveSplit(comp, auxSplit.mini),
                }
                : {}),
        };
    }
...
export function getAbilityRootsAndStems(info) {
    const isIntransitiveStativeCompound = (info.type === "stative compound" &&
...
function addAbilityHelperRootsAndStems(roots
// isIntransitiveStativeCompound: boolean
) {
    function addAbilityHelperToRoot(r, helper) {
        if ("mascSing" in r) {
            return {
                mascSing: addAbilityHelperToRoot(r.mascSing, helper),
                mascPlur: addAbilityHelperToRoot(r.mascPlur, helper),
                femSing: addAbilityHelperToRoot(r.femSing, helper),
                femPlur: addAbilityHelperToRoot(r.femPlur, helper),
            };
        }
        return {
...
function addAbilityHelperToPerfectiveSplit(s, helper) {
    if ("mascSing" in s) {
        return {
            mascSing: addAbilityHelperToPerfectiveSplit(s.mascSing, helper),
            mascPlur: addAbilityHelperToPerfectiveSplit(s.mascPlur, helper),
            femSing: addAbilityHelperToPerfectiveSplit(s.femSing, helper),
            femPlur: addAbilityHelperToPerfectiveSplit(s.femPlur, helper),
        };
    }
    return {
...
export function getPassiveRootsAndStems(info, withTails) {
    if (info.transitivity === "intransitive")
        return undefined;
    return {
...
const passiveRootTail = { p: "ی", f: "ay" };
function getPassiveStem(root, splitInfo, withTails) {
...
function getPassiveStemPerfectiveSplit(stem, splitInfo) {
    const si = "long" in splitInfo ? splitInfo.long : splitInfo;
    if ("mascSing" in si) {
        if (!("mascSing" in stem))
            throw new Error("persInflections doesn't match perfective split");
        return {
            mascSing: getPassiveStemPerfectiveSplit(stem.mascSing, si.mascSing),
            mascPlur: getPassiveStemPerfectiveSplit(stem.mascPlur, si.mascPlur),
            femSing: getPassiveStemPerfectiveSplit(stem.femSing, si.femSing),
            femPlur: getPassiveStemPerfectiveSplit(stem.femPlur, si.femPlur),
        };
    }
    return [
        si[0],
...
function getPassiveRootPerfectiveSplit(root, splitInfo) {
    const si = "long" in splitInfo ? splitInfo.long : splitInfo;
    if ("mascSing" in si) {
        if (!("mascSing" in root))
            throw new Error("persInflections doesn't match perfective split");
        return {
            mascSing: getPassiveRootPerfectiveSplit(root.mascSing, si.mascSing),
            mascPlur: getPassiveRootPerfectiveSplit(root.mascPlur, si.mascPlur),
            femSing: getPassiveRootPerfectiveSplit(root.femSing, si.femSing),
            femPlur: getPassiveRootPerfectiveSplit(root.femPlur, si.femPlur),
        };
    }
    return {
        short: [
...
function getAbilityRoots(root, isIntransitiveStativeCompound) {
    function getAspectAbilityRoot(root, aspect) {
        if ("mascSing" in root) {
            return {
                mascSing: getAspectAbilityRoot(root.mascSing, aspect),
                mascPlur: getAspectAbilityRoot(root.mascPlur, aspect),
                femSing: getAspectAbilityRoot(root.femSing, aspect),
                femPlur: getAspectAbilityRoot(root.femPlur, aspect),
            };
        }
        return {
            long: concatPsString(root.long, abilityTail),
            short: concatPsString(root.short, aspect === "imperfective" ? abilityTailAccented : abilityTail),
        };
    }
    function getAbilityRootPerfectiveSplit(s) {
        if ("mascSing" in s) {
            return {
                mascSing: getAbilityRootPerfectiveSplit(s.mascSing),
                mascPlur: getAbilityRootPerfectiveSplit(s.mascPlur),
                femSing: getAbilityRootPerfectiveSplit(s.femSing),
                femPlur: getAbilityRootPerfectiveSplit(s.femPlur),
            };
        }
        return {
    ...
    return {
        perfective: getAspectAbilityRoot(!isIntransitiveStativeCompound ? root.perfective : root.imperfective, !isIntransitiveStativeCompound ? "perfective" : "imperfective"),
...
function getPassiveRoot(root, splitInfo, withTails) {
    const perfectiveRoot = withTails
        ? concatPsString(root.perfective, passiveRootTail)
        : root.perfective;
    const imperfectiveRoot = withTails
...
function getPassivePastParticiple(root, withTails) {
    if ("mascSing" in root) {
        return {
            mascSing: getPassivePastParticiple(root.mascSing),
            mascPlur: getPassivePastParticiple(root.mascPlur),
            femSing: getPassivePastParticiple(root.femPlur),
            femPlur: getPassivePastParticiple(root.femPlur),
        };
    }
    const r = withTails ? concatPsString(root, passiveRootTail) : root;
    // @ts-expect-error because
    return concatPsString(removeAccents(getLong(r)), " ", stativeAux.intransitive.info.participle.past);
}
function getPassiveStemAspect(root, aspect) {
    if ("mascSing" in root) {
        return {
            mascSing: getPassiveStemAspect(root.mascSing, aspect),
            mascPlur: getPassiveStemAspect(root.mascPlur, aspect),
            femSing: getPassiveStemAspect(root.femPlur, aspect),
            femPlur: getPassiveStemAspect(root.femPlur, aspect),
        };
    }
    return concatPsString(aspect === "imperfective" ? removeAccents(getLong(root)) : getLong(root), " ", stativeAux.intransitive.info.stem[aspect]);
}
function getPassiveRootAspect(root, aspect) {
    if ("mascSing" in root) {
        return {
            mascSing: getPassiveRootAspect(root.mascSing, aspect),
            mascPlur: getPassiveRootAspect(root.mascPlur, aspect),
            femPlur: getPassiveRootAspect(root.femPlur, aspect),
            femSing: getPassiveRootAspect(root.femPlur, aspect),
        };
    }
    const rootR = aspect === "imperfective" ? removeAccents(root.long) : root.long;
    return {
        long: concatPsString(rootR, " ", 
        // @ts-expect-error because
        stativeAux.intransitive.info.root[aspect].long),
        short: concatPsString(rootR, " ", 
        // @ts-expect-error because
