...
export function accentOnFront(s) {
    if ("long" in s) {
        return {
 ...
 */
export function accentPastParticiple(s) {
    // check for accent placing in words like wáayulay and azmóyulay
    const accentFallsOnThirdLast = (syls) => {
        if (syls.length < 3)
            return false;
...
export function splitUpSyllables(f) {
    return (f.match(/ |([^a|á|e|é|i|í|o|ó|u|ú| ]*(aa|áa|a|á|ay|áy|ee|ée|e|é|oo|óo|o|ó|i|í|u|ú)[^a|á|e|é|i|í|o|ó|u|ú| ]*)/gi) || []);
}
export function countSyllables(f) {
    if (typeof f !== "string")
        return countSyllables(f.f);
 ...
 */
export function accentFSylsOnNFromEnd(syls, n) {
...
export function accentOnNFromEnd(ps, n) {
...
export function accentLetter(s) {
    if (!s) {
 ...
 * @param ps
 */
export function getAccentPos(ps) {
...
export function accentIsOnEnd(ps) {
    return getAccentPos(ps) === 0;
}
export function accentPsSyllable(ps) {
    return {
        p: ps.p,
        f: accentLetter(ps.f),
    };
}
export function removeAccentsWLength(s) {
    if ("long" in s) {
        return {
            long: removeAccentsWLength(s.long),
            short: removeAccentsWLength(s.short),
            ...(s.mini
                ? {
                    mini: removeAccentsWLength(s.mini),
                }
                : {}),
        };
    }
