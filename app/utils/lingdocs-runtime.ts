...
export class LingDocsRuntime {
    ...
    async conjugateVerb(verb: string): Promise<VerbConjugationResult> {
        const conjugationModule = await this.loadModule('./lingdocs/verb-conjugation');
        return conjugationModule.conjugateVerb(verb);
    }

    async inflectWord(word: string): Promise<string[]> {
        const inflectorModule = await this.loadModule('./lingdocs/pashto-inflector');
        return inflectorModule.inflectWord(word);
    }
}
