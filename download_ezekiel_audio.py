#!/usr/bin/env python3
"""
Download Ezekiel audio files from Afghan Bibles and split into individual verses.

This script focuses specifically on Ezekiel chapters and downloads all available
audio files, splitting them into individual verse files.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ezekiel_audio_download.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('ezekiel_audio_downloader')
...
class EzekielAudioDownloader:
    """Download and process Ezekiel audio files from Afghan Bibles"""
    ...
    def __init__(self, audio_dir: str = "ot_audio_files"):
        ...
        self.session.headers.update({
            'User-Agent': 'PashtoBibleSearch-Ezekiel-Audio-Downloader/1.0 (+https://pashto-bible-search.vercel.app/)'
        })
    ...
    def get_chapter_count(self, book_slug: str) -> int:
        ...
        try:
            ...
            # For Ezekiel, we know it has 48 chapters
            if book_slug == "ezekiel":
                return 48
    ...
    async def download_ezekiel_chapters(self, start_chapter: int = 1, end_chapter: int = None):
        """Download all available Ezekiel chapters"""
        book_slug = "ezekiel"
        total_chapters = self.get_chapter_count(book_slug)
        end_chapter = end_chapter or total_chapters

        logger.info(f"Starting Ezekiel audio download: chapters {start_chapter}-{end_chapter} of {total_chapters}")

        downloaded_count = 0
        processed_count = 0

        for chapter in range(start_chapter, min(end_chapter, total_chapters) + 1):
            logger.info(f"\n=== Processing Ezekiel Chapter {chapter} ===")

            # Check if audio exists
            if not self.check_chapter_audio_exists(book_slug, chapter):
                logger.warning(f"No audio file found for Ezekiel {chapter}")
                continue

            # Download chapter audio file
            content_hash = self.download_audio_file(book_slug, chapter)
            if not content_hash:
                logger.error(f"Failed to download Ezekiel {chapter}")
                continue
            ...
            if chapter_info and chapter_info['jktags']:
                ...
                if markers:
                    ...
                    if verse_files:
                        logger.info(f"✅ Successfully processed Ezekiel Chapter {chapter}: {len(verse_files)} verses")
                        processed_count += 1
                    else:
                        logger.warning(f"❌ Failed to split audio for Ezekiel {chapter}")
                else:
                    logger.warning(f"No timing markers found for Ezekiel {chapter}")
            else:
                logger.warning(f"No jktags found for Ezekiel {chapter}")

            # Respectful delay between chapters
            await asyncio.sleep(1.0)

        logger.info(f"\n🎉 Ezekiel download complete!")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download Ezekiel audio files from Afghan Bibles')
    ...
    downloader = EzekielAudioDownloader(args.audio_dir)

    asyncio.run(downloader.download_ezekiel_chapters(args.start_chapter, args.end_chapter))
